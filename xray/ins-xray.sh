...
# install nginx
mkdir -p /home/vps/public_html
wget -q -O /etc/nginx/conf.d/vps.conf "${REPO}configurasion/vps.conf.txt"
sleep 1
wget -q -O xraymode.sh ${REPO}xray/xraymode.sh && chmod +x xraymode.sh && ./xraymode.sh
sleep 1 
wget -q -O /etc/xray/config.json "${REPO}configuration/config.json"
chmod +x /etc/xray/config.json
sleep 1 
rm -f /etc/nginx/conf.d/xray.conf
wget -q -O /etc/nginx/conf.d/xray.conf "${REPO}configuration/xray.conf"
