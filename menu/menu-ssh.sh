...
export Auther="ColongVPN"

rm -f /usr/bin/lock
rm -f /usr/bin/unlock
wget -q -O /usr/bin/lock "${REPO}ssh/user-lock.sh" && chmod +x /usr/bin/lock
wget -q -O /usr/bin/unlock "${REPO}ssh/user-unlock.sh" && chmod +x /usr/bin/unlock
if [ "${EUID}" -ne 0 ]; then
